...
namespace InteractiveRangeSelect {
    ...
    public partial class Form1 : Form {
        ...
        private void PopulateDataSet () {
            ...
            ((XYDiagram)chartLine.Diagram).AxisX.DateTimeMeasureUnit = DateTimeMeasurementUnit.Minute;
            ((XYDiagram)chartLine.Diagram).AxisX.DateTimeGridAlignment = DateTimeMeasurementUnit.Minute;
            ((XYDiagram)chartLine.Diagram).AxisX.DateTimeOptions.Format = DateTimeFormat.Custom;
            ((XYDiagram)chartLine.Diagram).AxisX.DateTimeOptions.FormatString = "HH:mm";
        }
        ...
        private void chartSelectRange_MouseUp (object sender, MouseEventArgs e) {
            ...
            ((XYDiagram)chartLine.Diagram).AxisX.Range.SetMinMaxValues(minRange, maxRange);
        }
        ...
        private void chartSelectRange_CustomPaint (object sender, CustomPaintEventArgs e) {
            ...
            ControlCoordinates leftBottom = Diagram.DiagramToPoint(minRange, 
                Convert.ToDouble(Diagram.AxisY.Range.MinValue));
            ControlCoordinates leftTop = Diagram.DiagramToPoint(minRange, 
                Convert.ToDouble(Diagram.AxisY.Range.MaxValue));
            ControlCoordinates rightTop = Diagram.DiagramToPoint(maxRange, 
                Convert.ToDouble(Diagram.AxisY.Range.MaxValue));

            ControlCoordinates leftZero = Diagram.DiagramToPoint(Convert.ToDateTime(Diagram.AxisX.Range.MinValue), 
                Convert.ToDouble(Diagram.AxisY.Range.MaxValue));
            ControlCoordinates rightMax = Diagram.DiagramToPoint(Convert.ToDateTime(Diagram.AxisX.Range.MaxValue), 
                Convert.ToDouble(Diagram.AxisY.Range.MaxValue));
